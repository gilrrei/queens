...
class Scaler(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def transform(self, x_mat: np.ndarray):
    ...
    @abc.abstractmethod
    def inverse_transform_mean(self, x_mat: np.ndarray):
    ...
    @abc.abstractmethod
    def inverse_transform_std(self, x_mat: np.ndarray):
...
class StandardScaler(Scaler):
    ...
    def __init__(self):
        """Initiliasize standard scaler."""
        super().__init__()
        self.mean: np.ndarray
        self.standard_deviation: np.ndarray
    ...
    def inverse_transform_grad_mean(
        ...
        Args:
            grad_mean: gradient of the transformed mean
                                  function
            standard_deviation_input: standard deviation of the input data

        Returns:
            Inversely transformed gradient of
                                         the mean function
        """
...
class IdentityScaler(Scaler):
    ...
    def inverse_transform_grad_mean(self, grad_mean: np.ndarray, *_args) -> np.ndarray:
        """Conduct the inverse scaling of the mean gradient.
    ...
    def inverse_transform_grad_var(self, grad_var: np.ndarray, *_args) -> np.ndarray:
        """Conduct the inverse scaling of the variance gradient.
