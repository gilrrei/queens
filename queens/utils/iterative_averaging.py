...
NumericalValue: TypeAlias = np.ndarray | np.generic | int | float
AvergedValue: TypeAlias = np.ndarray | np.generic
...
class IterativeAveraging(metaclass=abc.ABCMeta):
    ...
    def __init__(self) -> None:
        ...
        self.current_average: NumericalValue = None
        self.new_value: NumericalValue = None
        self.rel_l1_change: float = 1
        self.rel_l2_change: float = 1

    def update_average(self, new_value: NumericalValue) -> AvergedValue:
        ...
        if self.current_average is not None:
            if isinstance(self.current_average, (np.generic, np.ndarray)):
    ...
    @abc.abstractmethod
    def average_computation(self, new_value: NumericalValue):
...
class MovingAveraging(IterativeAveraging):
    ...
    def average_computation(self, new_value: NumericalValue) -> AvergedValue:
        ...
        Returns:
            The current average
        """
        if isinstance(new_value, (np.generic, np.ndarray)):
...
class PolyakAveraging(IterativeAveraging):
    ...
    def __init__(self) -> None:
        ...
        self.sum_over_iter: NumericalValue = 0
...
def l1_norm(vector: NumericalValue, averaged: bool = False) -> float | np.generic:
...
def l2_norm(vector: NumericalValue, averaged: bool = False) -> float | np.generic:
    """Compute the L2 norm of the vector.

    Args:
        vector: Vector
        averaged: If enabled the norm is divided by the square root of the number of
                         components
